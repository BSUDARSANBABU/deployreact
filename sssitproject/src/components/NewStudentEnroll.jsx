...
export default function NewStudentEnroll() {
  ...
  const fetchData = async () => {
    try {
      const [facultyRes, studentsRes, coursesRes] = await Promise.all([
        axios.get(API_URLS.faculty),
        axios.get(API_URLS.students),
        axios.get(API_URLS.courses),
      ]);
  ...
  const handleAdd = async (e) => {
    ...
    // 🔍 FRONTEND DUPLICATE REG NO VALIDATION
    ...
    try {
      ...
      const res = await axios.post(API_URLS.students, fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      await axios.post(API_URLS.fees, {
        student: res.data.id,
  ...
  const handleLastRecord = () => {
    if (students.length === 0) return showMessage("No data found", "error");
    ...
    setPreview(
      last.image
        ? last.image.startsWith("http")
          ? last.image
          : `http://127.0.0.1:8000${last.image}`
        : null
    );

    showMessage("Last record loaded", "success");
  };
  const handleLastModified = () => {
    const lastId = localStorage.getItem("lastModifiedStudentId");
    ...
    const student = students.find(s => String(s.id) === String(lastId));

    if (!student) {
      showToast("Edited record not found", "error");
      return;
    }
  ...
  return (
    <div className="container-sm admin-enroll">
      {toast.text && (
