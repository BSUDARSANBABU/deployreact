...
export default function OldStudentEnroll() {
  ...
  const loadFaculty = async () => {
    try {
      const res = await axios.get(API_URLS.faculty);
  ...
  const loadCourses = async () => {
    try {
      const res = await axios.get(API_URLS.courses);
  ...
  const handleRegnoChange = async (e) => {
    ...
    if (student.image) {
      if (student.image.startsWith("http"))
        ...
        imageUrl = `http://127.0.0.1:8000${student.image}`;
      else
        imageUrl = `http://127.0.0.1:8000/media/${student.image}`;
    }

    setPreview(imageUrl);

    // USE OLD IMAGE DIRECTLY IF EXISTS
    if (imageUrl) {
  ...
  const handleCreateNewEnrollment = async (e) => {
    ...
    try {
      ...
      await axios.post(API_URLS.students, fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  // ---------------- UI -------------------
  return (
    <div className="container text-center admin-enroll">
      {toast.text && (
