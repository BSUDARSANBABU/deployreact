...
export default function FeeReceipts() {
  ...
  const loadReceipts = async (student) => {
    ...
    try {
      const res = await axios.get(`${API_URLS.fees}?student=${student.id}`);
  ...
  const handleLoadPrevious = async () => {
    ...
    try {
      const res = await axios.get(API_URLS.fees);
      const all = Array.isArray(res.data) ? res.data : [];
      // only consider receipts that belong to a student and have a receipt number
      const valid = all.filter((r) => r.student !== null && r.receipt_no);
      ...
      const sres = await axios.get(`${API_URLS.students}${studentId}/`);
      const student = sres.data;

      // compute totals/paid/due for the selected student
      const calc = await computePaidDueForStudent(student);
      const enhancedStudent = { ...student, ...calc };

      setSelected(enhancedStudent);
      setSelectedId(student.id);
      setRegNo(student.regno ?? "");

      // only display the last receipt record
  ...
  const computePaidDueForStudent = async (stu) => {
    try {
      const r = await axios.get(`${API_URLS.fees}?student=${stu.id}`);
  ...
  const fetchList = async (value) => {
    try {
      const res = await axios.get(`${API_URLS.students}?regno=${value}`);
  ...
  const handleAddReceipt = async () => {
    ...
    try {
      ...
      await axios.post(API_URLS.fees, payload);
  ...
  const handleEditSave = async () => {
    try {
      await axios.patch(`${API_URLS.fees}${editReceipt.id}/`, {
        student: selected.id,
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${API_URLS.fees}${deleteId}/`);
      setShowDeleteConfirm(false);

      await loadReceipts(selected);
      if (regNo) fetchList(regNo);
  ...
  const validReceipts = receipts.filter((r) => r.receipt_no !== null);
